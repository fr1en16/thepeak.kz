...
export function ScrollPortraitWall({
  ...
  const hasHeader = Boolean(title || date);
  ...
  return (
    ...
    >
      ...
      <div className={cn("relative z-0", hasHeader ? "mb-[50vh] mt-[50vh]" : "mb-20 mt-10")}>
        {layout.map((row, ri) => (
          <div key={ri} className="flex w-full">
            {row.map((idx, ci) => {
              if (idx === -1)
                return <div key={ci} className="aspect-video flex-1" />;

              const s = speakers[idx];
              const origin = ci < cols / 2 ? "right bottom" : "left bottom";

              return (
                <div key={ci} className="aspect-video flex-1">
                  <div
