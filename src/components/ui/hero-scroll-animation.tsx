...
interface SecondSectionProps extends AnimatedSectionProps {
...
function CoverSection({
  ...
  return (
    <motion.div
      ...
      style={animate ? { scale, rotate, filter } : undefined}
      className="relative z-0 origin-center lg:sticky lg:top-0 lg:will-change-[transform,filter]"
    >
      {children}
    </motion.div>
  );
...
const HeroScrollAnimation = forwardRef<
  HTMLDivElement,
...
>(({ cover, second }, forwardedRef) => {
  ...
  return (
    <div
      ref={setContainerRef}
      className="relative isolate col-span-12 w-full bg-black [perspective:1200px] before:absolute before:inset-y-0 before:left-1/2 before:-z-10 before:w-screen before:-translate-x-1/2 before:bg-black before:content-['']"
    >
