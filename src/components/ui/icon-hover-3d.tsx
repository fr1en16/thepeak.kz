...
export const SupportIcon = ({ isHovered }: { isHovered: boolean }) => (
  <div style={{ transformStyle: 'preserve-3d' }}>
    {[0, 60, 120].map((deg, i) => (
      <motion.div 
        key={i} 
        className={`absolute rounded-full border-[4px] ${i === 0 ? 'border-brand-gray' : 'border-brand-gray/50'}`} 
        style={{ width: 240, height: 240, x: -120, y: -120 }}
        initial={{ rotateY: deg }}
        animate={{ 
          rotateY: isHovered ? deg + 180 : deg,
...
export const MarketingCone = ({ isHovered }: { isHovered: boolean }) => {
  const sections = [
    { y1: 90, r1: 100, y2: 40, r2: 75 },   // Bottom frustum
    { y1: 25, r1: 65, y2: -25, r2: 40 },  // Middle frustum
    { y1: -40, r1: 30, y2: -90, r2: 2 }   // Top cone segment
  ];
  ...
  return (
    <div style={{ transformStyle: 'preserve-3d' }}>
      {sections.map((sec, sIdx) => {
        const dy = sec.y1 - sec.y2;
        const dz = sec.r1 - sec.r2;
        const L = Math.sqrt(dy * dy + dz * dz);
        const alpha = (Math.atan2(dz, dy) * 180) / Math.PI;

        return (
          <React.Fragment key={sIdx}>
            {/* Bottom Ring of section */}
            <div
              className="absolute rounded-full border-[4px] border-brand-gray"
              style={{
                width: sec.r1 * 2,
                height: sec.r1 * 2,
                left: "50%",
                top: "50%",
                transform: `translate3d(-50%, -50%, 0) translate3d(0px, ${sec.y1}px, 0px) rotateX(90deg)`,
            ...
            <div
              ...
              style={{
                ...
                left: "50%",
                top: "50%",
                transform: `translate3d(-50%, -50%, 0) translate3d(0px, ${sec.y2}px, 0px) rotateX(90deg)`,
                transformStyle: 'preserve-3d',
              }}
            />
            {/* Side lines */}
            {angles.map((theta) => (
              <div
                ...
                style={{
                  ...
                  transform: `translate3d(-50%, 0%, 0) rotateY(${theta}deg) translate3d(0px, ${sec.y2}px, ${sec.r2}px) rotateX(${alpha}deg)`,
                }}
              />
            ))}
          </React.Fragment>
...
export const IconHover3D: React.FC<Props> = ({
  ...
  const isOrganisation = heading === "Организация и сопровождение";
  const isMarketing = heading === "Маркетинг и стратегия";
          ...
          <Transition value={transition1}>
            <motion.div
              ...
              style={{
                ...
                position: "relative",
                width: "100%",
                borderRadius: "0px", // Standard Peak style is rounded-none
            ...
            >
                ...
                {isOrganisation ? (
                  ...
                  >
                    <motion.div
                      ...
                      animate={{
                        scale: isHoverVariant ? 1.1 : 0.7,
                      }}
                      transition={transition1}
                    >
                      <SupportIcon isHovered={isHoverVariant} />
                    </motion.div>
                    {/* Corner elements */}
                    <motion.div
                      style={{
                  ...
                  >
                    <motion.div
                      ...
                      animate={{
                        scale: isHoverVariant ? 1.1 : 0.7,
                        rotateX: isHoverVariant ? -28 : 23,
                        rotateY: isHoverVariant ? -43 + 360 : 33,
                        rotate: 49
                      }}
                      transition={transition1}
                    >
                      <MarketingCone isHovered={isHoverVariant} />
                    </motion.div>
                    {/* Corner elements */}
                    <motion.div
                      style={{
              ...
              >
                ...
                {/* Price */}
                {price && (
                  <div className="font-sans text-[clamp(0.8rem,0.95vw,1rem)] font-extrabold text-brand-red uppercase tracking-wider mt-auto pt-3 select-none">
                    {price}
                  </div>
                )}
              </motion.div>
