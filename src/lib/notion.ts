export async function getCaseData(slug: string) {
    console.log("DEBUG: Запрос к Notion для slug:", slug); // ЭТО ПОЯВИТСЯ В ЛОГАХ

    if (!process.env.NOTION_TOKEN) console.error("DEBUG: NOTION_TOKEN не найден!");
    if (!process.env.NOTION_DATABASE_ID) console.error("DEBUG: DATABASE_ID не найден!");
