...
Локальная папка `/public/cases` используется только для локальных фото кейсов и рабочей копии видео. Локальные видео из этой папки не читаются API галереи и не должны попадать в репозиторий.
...
Поддерживаемые локальные форматы для галереи: `.jpg`, `.jpeg`, `.png`, `.webp`, `.avif`, `.gif`.

Локальные видео форматы `.mp4`, `.webm`, `.mov`, `.m4v` игнорируются в Git и не используются как источник галереи.
...
3. Если Cloudinary не настроен, вернул ошибку или не нашел видео, брать видео из статического manifest-файла `src/data/case-media-manifest.ts`.
4. Локально из `public/cases/{slug}` читать только фото.
5. Возвращать локальные URL фото в формате:
